...
def do_work(interval):
  ...
  if IS_JOURNALD:
    ...
    p0 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "0..0"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
    p1 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "1..1"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
    p2 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "2..2"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
    p3 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "3..3"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
    p4 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "4..4"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
    p5 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "5..5"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
    p6 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "6..6"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
    p7 = len(subprocess.Popen(["journalctl", since, "--no-pager", "-p", "7..7"], stdout=subprocess.PIPE).stdout.read().splitlines()) - 0.9
  else:
