...
pushd "$HOME/lnxdiagd/queries/" >/dev/null || exit 1
  ...
  if [[ $(find "${DATASTORE}/sql11h.csv" -mmin +$AGE) || ! -f "${DATASTORE}/sql11h.csv" ]]; then
  ...
  if [[ $(find "${DATASTORE}/sql12h.csv" -mmin +$AGE)  || ! -f "${DATASTORE}/sql12h.csv" ]]; then
  	echo -n "12"
    time mysql -h sql --skip-column-names -e            \
    "USE domotica;                                 \
     SELECT MIN(sample_epoch),                     \
  ...
  if [[ $(find "${DATASTORE}/sql13h.csv" -mmin +$AGE)  || ! -f "${DATASTORE}/sql13h.csv"  ]]; then
  	echo -n "13"
    time mysql -h sql --skip-column-names -e            \
    "USE domotica;                                  \
     SELECT MIN(sample_epoch),                      \
  ...
  if [[ $(find "${DATASTORE}/sql14h.csv" -mmin +$AGE)  || ! -f "${DATASTORE}/sql14h.csv"  ]]; then
  	echo -n "14"
    time mysql -h sql --skip-column-names -e            \
    "USE domotica;                                 \
     SELECT MIN(sample_epoch),                     \
  ...
  if [[ $(find "${DATASTORE}/sql15h.csv" -mmin +$AGE)  || ! -f "${DATASTORE}/sql15h.csv"  ]]; then
    echo -n "15"
    time mysql -h sql --skip-column-names -e            \
    "USE domotica;                                 \
     SELECT MIN(sample_epoch),                     \
  ...
  if [ "${HOST}" == "boson" ]; then
    ...
    # multiply H_DIVIDER by 5 because sampling takes place every 300s (5*60s)
    if [[ $(find "${DATASTORE}/sql19h.csv" -mmin +$AGE)  || ! -f "${DATASTORE}/sql19h.csv"  ]]; then
      echo -n "19"
      time mysql -h sql --skip-column-names -e            \
      "USE domotica;                                    \
        SELECT                                          \
